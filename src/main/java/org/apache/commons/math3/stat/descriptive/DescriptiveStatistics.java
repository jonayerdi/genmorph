...
public class DescriptiveStatistics implements StatisticalSummary, Serializable {
    ...
    public static final String CSVHEADER = "n,min,max,mean,stddev,median,skewness,kurtosis";

    public String toCSV() {
        final StringBuilder outBuffer = new StringBuilder();
        final String endl = ",";
        outBuffer.append(getN()).append(endl);
        outBuffer.append(getMin()).append(endl);
        outBuffer.append(getMax()).append(endl);
        outBuffer.append(getMean()).append(endl);
        outBuffer.append(getStandardDeviation())
                .append(endl);
        try {
            // No catch for MIAE because actual parameter is valid below
            outBuffer.append(getPercentile(50)).append(endl);
        } catch (final MathIllegalStateException ex) {
            outBuffer.append("NA").append(endl);
        }
        outBuffer.append(getSkewness()).append(endl);
        outBuffer.append(getKurtosis()).append("\n");
        return outBuffer.toString();
    }
