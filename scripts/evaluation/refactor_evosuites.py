...
def refactor_code(infile, outfile, clazz_renames):
    with open(infile, mode='r') as infp, open(outfile, mode='wb') as outfp:
        for line in infp:
            # Skip @RunWith(EvoRunner.class) line
            if line.lstrip().startswith('@RunWith('):
                continue
            # Remove scaffolding
            line = re.sub(r'public class (.+) extends (.+) \{', lambda m: 'public class ' + m.group(1) + ' {', line)
            # Rename class
    ...
    while search:
        ...
        for child in listdir(current):
            ...
            if isdir(c):
                ...
                if c.endswith('_ESTest.java'):
                    yield c

def refactor_evosuites(in_dir, out_dir):
...
def main():
    ...
    import re
    if len(sys.argv) != 3:
        print('./refactor_evosuites.py <IN_DIR> <OUT_DIR>')
        exit(1)
    in_dir = sys.argv[1]
    out_dir = sys.argv[2]
    refactor_evosuites(in_dir=in_dir, out_dir=out_dir)
